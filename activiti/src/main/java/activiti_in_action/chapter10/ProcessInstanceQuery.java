...
public class ProcessInstanceQuery {
	...
	public static void main(String[] args) throws Exception {
		...
		Deployment deploy = repositoryService.createDeployment()
				.addClasspathResource("activiti_in_action/chapter10/ProcessInstanceQuery.bpmn").deploy();
		// 查询流程定义 
		...
		// 查询执行流
		Execution exe1 = runtimeService.createExecutionQuery().activityId("usertask1")
				.processInstanceId(pi1.getId()).singleResult();
		Execution exe2 = runtimeService.createExecutionQuery().processInstanceId(pi2.getId())
...
//		taskService.complete(task.getId());
		...
		exe1 = runtimeService.createExecutionQuery().processInstanceId(pi1.getId())
				.activityId("usertask2").singleResult();
		runtimeService.trigger(exe1.getId());
